   ...
   [obmap, minx, miny, maxx, maxy, xw, yw] = calc_obstacle_map(ox, oy, reso, vr);
    ...
    while true
     ...
     for i = 1:nmotion               %从当前节点，扩大搜索栅格，依次判断这八个点
         ...
         closed(num2str(node_ind))  
             continue
         end
         if open.isKey(num2str(node_ind))              % 判断是否在开集中，在，继续判断
             node1 = open(num2str(node_ind))  ;
             if node1.cost > node.cost    % 如果开集中该点的cost大于之前的cost
                 node1.cost = node.cost;  %
                 node1.pind = c_id;       % 更新该点的父节点
                 open(num2str(node_ind)) = node1 ;
             end
         else
