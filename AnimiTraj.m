...
function AnimiTraj(xp, N, ego, L, nOb, vOb, lOb, disp_title, plotNumb):
    ...
    figure(12);%figure(figsize=(44*10/44, 34*10/44))
    axis([-22, 22,-12, 22])
    ...
    def init():
        ...
        for j in range(nOb):
            for k in range(0, vOb[j]):
                plot([lOb[j][k][0],lOb[j][k+1][0]] , [lOb[j][k][1],lOb[j][k+1][1]] ,"k")
            end
        end

        plot(x0[0, 0], x0[1, 0], "ob")
        plot(xF[0, 0], xF[1, 0], "ob")
        ...
        function lines = update(i)

            Rot = [np.cos(xp(i, 2)), -np.sin(xp(i, 2));
    ...
    function [x,y]= carBox(x0, phi, w, l)

        car1 = x0(0:2] + [np.cos(phi) * l, np.sin(phi) * l] + [np.sin(phi) * w, -np.cos(phi) * w];
        car2 = x0(0:2] + [np.cos(phi) * l, np.sin(phi) * l] - [np.sin(phi) * w, -np.cos(phi) * w];
        car3 = x0(0:2] - [np.cos(phi) * l, np.sin(phi) * l] + [np.sin(phi) * w, -np.cos(phi) * w];
        car4 = x0(0:2] - [np.cos(phi) * l, np.sin(phi) * l] - [np.sin(phi) * w, -np.cos(phi) * w];
